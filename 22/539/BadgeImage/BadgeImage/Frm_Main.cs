    ...
    public partial class Frm_Main : Form
    {
        ...
        public Image SetBadge(PictureBox Pict, String Str, Font font, int place)
        {
            ...
            switch (place)//選擇文字的顯示位置
            {
                case 1://右下角
                    {
                        Var_StrX = (int)(Var_bmp.Width - Var_Size.Width - 3);//設定文字的X坐標值
                        Var_StrY = (int)(Var_bmp.Height - Var_Size.Height);//設定文字的Y坐標值
                        break;
                    }
                case 2://右上角
                    {
            ...
            for (int x = 1; x < Var_bmp.Width; x++)
            {
                for (int y = 1; y < Var_bmp.Height; y++)
                {
                    ...
                    if (Var_bmp.GetPixel(x, y).ToArgb() == Color.Black.ToArgb())//如果目前象素的顏色為黑色
                    {
                        ...
                        tem_r1 = tem_r;//臨時存儲R色值
                        tem_g1 = tem_g;//臨時存儲G色值
                        tem_b1 = tem_b;//臨時存儲B色值
                        //根據加深後的圖片背景顯示文字
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            pictureBox1.Image = SetBadge(pictureBox1, textBox1.Text, V_font, V_p);//呼叫自定義方法
        }
