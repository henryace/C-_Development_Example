...
namespace ImgMicroimage
{
    public partial class Frm_Main : Form
    {
        ...
        public bool GetReducedImage(double Percent, string targetFilePath)
        {
            try
            {
                Bitmap bt = new Bitmap(120, 120);							//建立Bitmap實例
                Graphics g = Graphics.FromImage(bt);						//建立Graphics實例
                g.Clear(Color.White);									//設定畫布背景顏色為白色
                Image ReducedImage;									//縮略圖
                Image.GetThumbnailImageAbort callb = new Image.GetThumbnailImageAbort(ThumbnailCallback);
                ImageWidth = Convert.ToInt32(ResourceImage.Width * Percent);		//設定寬度
                ImageHeight = Convert.ToInt32(ResourceImage.Height * Percent);	//設定高度
                //取得所謂圖
                ...
                bt.Save(@targetFilePath, ImageFormat.Jpeg);					//儲存縮略圖
                bt.Dispose();											//釋放物件
                ReducedImage.Dispose();								//釋放物件
                return true;
            }
            catch (Exception e)
            {
        ...
        private void a()
        {
            ...
            for (int i = 0; i < fi.Length; i++)
            {
                ...
                tsslText.Text = "共有" + this.clv.Items.Count.ToString() + "張圖片";
            }
            td.Abort();
        }
