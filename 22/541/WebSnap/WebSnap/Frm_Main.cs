    ...
    public partial class Frm_Main : Form
    {
        ...
        private void Snapweb()
        {
            ...
            while (lineheight < webBrowser1.Document.Body.ScrollRectangle.Height - 199)
            {
                ...
                {
                    inputHide();
                    BitBlt(DestDC, 0, 0, webBrowser1.Document.Body.ClientRectangle.Width, webBrowser1.Document.Body.ClientRectangle.Height, SourceDC, Rectangles.Left, Rectangles.Top, 13369376);//取得可見區域場景
                    linewidth = webBrowser1.Document.Body.ClientRectangle.Width + 5; //賦值寬
                ...
                {
                    inputHide();
                    BitBlt(DestDC, 0, webBrowser1.Document.Body.ClientRectangle.Height + 199 * i, webBrowser1.Document.Body.ClientRectangle.Width, 199, SourceDC, Rectangles.Left, Rectangles.Top + webBrowser1.Document.Body.ClientRectangle.Height - 199, 13369376);//取得可見區域場景
                    linewidth = webBrowser1.Document.Body.ClientRectangle.Width;//目前寬度
                    if (linewidth < webBrowser1.Document.Body.ScrollRectangle.Width)
                    {
                        while (linewidth < webBrowser1.Document.Body.ScrollRectangle.Width - 199)
                        {
                ...
                webBrowser1.Document.Body.ScrollTop += 199;//調整縱向滾動條位置
            }
            if (lineheight >= webBrowser1.Document.Body.ScrollRectangle.Height - 199)
            {
                ...
                if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    ig.Save(path);
                    MessageBox.Show("儲存成功", "提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    webBrowser1.Document.Body.ScrollTop = 0;
                    ig.Dispose();
                }
            }
