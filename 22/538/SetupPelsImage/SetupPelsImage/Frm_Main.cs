...
namespace SetupPelsImage
{
    public partial class Frm_Main : Form
    {
        ...
        int Var_MouseX = 0;//記錄鼠標的X坐標的位置
        int Var_MouseY = 0;//記錄鼠標的X坐標的位置
        int Var_MouseW = 0;//記錄鼠標拖動矩形框的寬度
        int Var_MouseH = 0;//記錄鼠標拖動矩形框的高度

        private void pictureBox1_MouseDown(object sender, MouseEventArgs e)
        {
            Var_MouseX = e.X;//記錄鼠標按下時的X坐標值
            Var_MouseY = e.Y;//記錄鼠標按下時的Y坐標值
        }

        private void pictureBox1_MouseUp(object sender, MouseEventArgs e)
        {
            Var_MouseW = e.X - Var_MouseX;//取得鼠標拖動矩形框的寬度
            Var_MouseH = e.Y - Var_MouseY;//取得鼠標拖動矩形框的高度
            if (Var_MouseW == 0)//矩形框的寬度為0
                Var_MouseW = 1;//設定寬度為1
            if (Var_MouseH == 0)//矩形框的高度為0
                Var_MouseH = 1;//設定高度為1
            label_X.Text = Var_MouseX.ToString();//顯示X的坐標值
            label_Y.Text = Var_MouseY.ToString();//顯示Y的坐標值
        ...
        {
            if (colorDialog1.ShowDialog() == DialogResult.OK)//打開顏色對話框
