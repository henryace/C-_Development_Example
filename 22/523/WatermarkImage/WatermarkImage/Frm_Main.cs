...
namespace WatermarkImage
{
    public partial class Frm_Main : Form
    {
        ...
        public Image pp(PictureBox Pict, int x, int y, int R, float better)//R強光照射面的半徑，即」光暈」
        {
            ...
            int tem_W = Var_Bmp.Width;//取得圖像的寬度
            int tem_H = Var_Bmp.Height;//取得圖像的高度
            //定義一個Bitmap類的復本
            ...
            for (int i = tem_W - 1; i >= 1; i--)
            {
                for (int j = tem_H - 1; j >= 1; j--)
                {
                    ...
                    if (Var_Length < R)
                    {
                        ...
                        r = Math.Max(0, Math.Min(r, 255));//如果R值不在顏色值的範圍內，對R值進行設定
                        g = Var_Color.G + (int)Var_Pixel;//設定加強後的G值
                        g = Math.Max(0, Math.Min(g, 255));//如果G值不在顏色值的範圍內，對G值進行設定
                        b = Var_Color.B + (int)Var_Pixel;//設定加強後的B值
                        b = Math.Max(0, Math.Min(b, 255));//如果B值不在顏色值的範圍內，對B值進行設定
