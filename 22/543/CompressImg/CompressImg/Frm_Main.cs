...
namespace CompressImg
{
    public partial class Frm_Main : Form
    {
        ...
        /// <param name="dFile">縮放後圖片的儲存路徑</param>
        /// <param name="dHeight">縮放後圖片的高度</param>
        /// <param name="dWidth">縮放後圖片的寬度</param>
        /// <returns></returns>
        ...
        public static bool GetPicThumbnail(string sFile, string dFile, int dHeight, int dWidth)
        {
            ...
            try
            {
                //取得包含有關內置圖像編碼解碼器的訊息的ImageCodecInfo對象。
                ...
                for (int x = 0; x < arrayICI.Length; x++)
                {
                    ...
                    {
                        jpegICIinfo = arrayICI[x];//設定JPEG編碼
                        break;
                    }
                }
                if (jpegICIinfo != null)
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    MessageBox.Show("警告：請選擇待處理的圖片目錄及儲存位置", "警告", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    ...
                    if (txtSavePath.Text.Trim() == "")
                        MessageBox.Show("警告：請選擇儲存路徑", "警告", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
