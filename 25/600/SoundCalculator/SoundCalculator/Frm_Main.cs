    ...
    public partial class Frm_Main : Form
    {
        ...
        public void GetVox()
        {
            ...
            if (System.IO.File.Exists(Application.StartupPath + "\\Tem_File.ini") == true)
            {
                ...
                {
                    GetPrivateProfileString("Vox", i.ToString(), "資料讀取錯誤。", temp, 255, Application.StartupPath + "\\Tem_File.ini");
        ...
        {
            tem_Value = ((PictureBox)sender).AccessibleName;//取得目前按鈕的標識
            switch (tem_Value)
            {
                case "0": num(tem_Value); sound(VoxPath[0]); break;//實現按鈕的語音功能
                ...
                case "+-":
                    {
                        zf();
                        if (Convert.ToInt32(textBox1.Text) > 0)//如果目前為正數
                            sound(VoxPath[21]);//實現正數發音
                        else
                            sound(VoxPath[22]);//實現負數發音
                        break;
                    }
        ...
        public void SetButton(object sender, bool b)//b為false時變色
        {
