...
namespace TextInCaky
{
    public partial class Frm_Main : Form
    {
        ...
        public static string[] XText = new string[7] { "商品1", "商品2", "商品3", "商品4", "商品5", "商品6", "商品7" };//存儲資料的名稱陣列
        ...
        public static string[] AreaText;//臨時存儲資料的名稱陣列
        Pen mypen;
        float AreaAngle = 0F;
        public static float XSize = 50;//X軸的大小
        public static float YSize = 50;//Y軸的大小
        public static float ASum = 50;//記錄餅型的總和
        ...
        {
            string temTextSize = "";//存儲最長的名稱
            Font LSfont = new System.Drawing.Font("細明體", 8);//設定說明文字的字體
            AreaText = new string[XText.Length];//實例化一個臨時陣列
        ...
        public void ProtractArea(Graphics g)
        {
            ...
            float AXLeft = 0;//設定圓餅圖的X坐標
            float AYUp = 0;//設定圓餅圖的Y坐標
            float AXSize = 0;//設定圓餅圖的寬度
            float AYSize = 0;//設定圓餅圖的高度
            float Atop = 0;//記錄餅型的高度為長和寬的最小值
            float Aleft = 0;//記錄餅型的高度為長和寬的最小值
            TimeNum = AreaAngle;//設定圓餅圖的起始度數
            //計算圓餅圖的初始位置
            XLeft = panel1.Width - (panel1.Width - 5);//去了邊框後圓餅圖的X位置
            XSize = panel1.Width - 10;//設定圓餅圖的寬度
            temXLeft = AXLeft;//記錄圓餅圖的X坐標
            AXLeft = XLeft;//記錄圓餅圖的X坐標
            AXSize = XSize;//記錄圓餅圖的寬度

            //透過說明文字的大小計算圓餅圖的位置
            AXLeft = AXLeft + AreaXMaxWidth + Aline;//設定去除說明文字後的圓餅圖X坐標
            AYUp = AYUp + AreaXMaxHeight;//設定去除說明文字後的圓餅圖Y坐標
            AXSize = XSize - AreaXMaxWidth * 2 - Aline * 2;//設定去除說明文字後的圓餅圖寬度
            AYSize = YSize - AreaXMaxHeight * 2;//設定去除說明文字後的圓餅圖高度
            if (AXSize >= AYSize)//如果圓餅圖的寬度大於等於高度
            {
                Aleft = AXSize - AYSize;//記錄圓餅圖的X坐標
                AXSize = AYSize;//將高度設為寬度
            }
            else
            {
                Atop = AYSize - AXSize;//記錄圓餅圖的Y坐標
                AYSize = AXSize;//將寬度設為高度
            }
            if (Aleft != 0)//如果寬大於高
            {
                AXLeft = AXLeft + Aleft / 2;//設定圓餅圖橫向局中
            }
            if (Atop != 0)//如果高大於寬
            {
                AYUp = AYUp + Atop / 2;//設定圓餅圖縱向局中
            }
            temXLeft = XLeft;
            //繪製圓餅圖
            if (AXSize > 0 && AYSize > 0)//如果圓餅圖的寬和高大於0
            {
                for (int i = 0; i < SzData.Length; i++)//深度搜尋資料
                {
                    f = SzData[i] / ASum;//取得目前資料的百分比
                    //設定目前扇形圖的填充顏色
                    if (i >= WearColor.Length)//如果沒有超出顏色陣列
        ...
        public void ProAreaSign(Graphics g)
        {
            ...
            {
                for (int i = 0; i < SzData.Length; i++)//深度搜尋所有說明文字
                {
