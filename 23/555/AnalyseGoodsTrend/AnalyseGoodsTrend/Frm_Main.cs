...
namespace AnalyseGoodsTrend
{
    public partial class Frm_Main : Form
    {
        ...
        {
            using (SqlConnection Con = new SqlConnection("server=(local);uid=sa;pwd=123456;database=db_TomeOne"))
            {
        ...
        private void CreateImage(int Year)
        {
            ...
            try
            {
                ...
                Font font1 = new System.Drawing.Font("細明體", 20, FontStyle.Regular);
                ...
                g.DrawString("" + Year + " 年某商品走勢", font1, brush1, new PointF(180, 30));
                //畫圖片的邊框線
                ...
                for (int i = 0; i < 11; i++)
                {
                    g.DrawString(m[i].ToString(), font, Brushes.Red, 25, y); //設定文字內容及輸出位置
                    y = y + 26;
                }

                int[] Count = new int[12];
                string cmdtxt2 = "SELECT * FROM tb_Stat WHERE ShowYear=" + Year + "";
                SqlConnection Con = new SqlConnection("server=(local);uid=sa;pwd=123456;database=db_TomeOne");
                Con.Open();
