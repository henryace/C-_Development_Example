...
public class PinYin
{
    ...
    /// 將中文字轉換拼音的方法
    /// </summary>
    /// <param name="str">中文字字串</param>
    /// <returns>拼音字串</returns>
    public string GetABC(string str)
    {
        Regex reg = new Regex("^[\u4e00-\u9fa5]$");//驗證輸入是否為中文字
        byte[] arr = new byte[2];//定義字節陣列
        string pystr = "";//定義字串變數用於返回拼音
        char[] mChar = str.ToCharArray();//取得中文字對應的字符陣列
    ...
    private string GetStr(char[] mChar, string pystr, Regex reg, byte[] arr)
    {
        ...
        {
            if (reg.IsMatch(mChar[j].ToString()))//如果輸入的是中文字
            {
