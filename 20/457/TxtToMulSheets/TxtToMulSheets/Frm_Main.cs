...
namespace TxtToMulSheets
{
    public partial class Frm_Main : Form
    {
        ...
        private void btn_Read_Click(object sender, EventArgs e)
        {
            ...
            Microsoft.Office.Interop.Excel.Worksheet newWorksheet;//聲明工作表對像
            for (int i = 0; i < P_str_List.Count; i++)//深度搜尋泛型集合
            {
                P_str_Content = P_str_List[i];//記錄深度搜尋到的值            
                newWorksheet = (Microsoft.Office.Interop.Excel.Worksheet)workbook.Worksheets.Add(missing, missing, missing, missing);//建立新工作表
                newWorksheet.Cells[1, 1] = P_str_Content;//直接將深度搜尋到的內容新增到工作表中
            }
            excel.Application.DisplayAlerts = false;//不顯示提示對話框
            workbook.Save();//儲存工作表
            workbook.Close(false, missing, missing);//關閉工作表
            MessageBox.Show("已經將文字文件的內容分解到了Excel的不同資料表中！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
