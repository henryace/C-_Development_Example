        ...
        private void btn_Excel_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgv_Info.RowCount - 1; i++)//深度搜尋資料表格控制元件的所有行
            {
                for (int j = 0; j < dgv_Info.ColumnCount; j++)//深度搜尋資料表格控制元件的所有列
                {
                    ...
                    {
                        excel.Cells[i + 2, j + 1] = "「" + dgv_Info[j, i].Value.ToString().Trim() + "」";//格式化為字串類型並填充Excel表格
                    }
                    else if (dgv_Info[j, i].ValueType == typeof(Int16) || dgv_Info[j, i].ValueType == typeof(Int32))//判斷深度搜尋到的資料是否是16位或者32位數字類型
                    {
                        excel.Cells[i + 2, j + 1] = Convert.ToInt32(dgv_Info[j, i].Value).ToString("N");//格式化為32位數字類型並填充Excel表格
                    }
                    else if (dgv_Info[j, i].ValueType == typeof(Int64))//判斷深度搜尋到的資料是否是64位數字類型
                    {
                        excel.Cells[i + 2, j + 1] = Convert.ToInt64(dgv_Info[j, i].Value).ToString("N");//格式化為64位數字類型並填充Excel表格
                    }
                    else if (dgv_Info[j, i].ValueType == typeof(DateTime))//判斷深度搜尋到的資料是否是日期類型
                    {
                        excel.Cells[i + 2, j + 1] = Convert.ToDateTime(dgv_Info[j, i].Value).ToLongDateString();//格式化為長日期形式並填充Excel表格
                    }
                    else
                    {
