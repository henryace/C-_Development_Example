...
namespace FindKey
{
    public partial class Frm_Main : Form
    {
        ...
        {
            richTextBox1.ReadOnly = true;//設定文字為只讀格式
            string filePath = "test.txt";//設定打開文件的路徑
            if (File.Exists(filePath)) //當存在該文件時
            {
                FetchFile(filePath);//在RichTextBox控制元件中顯示指定路徑下的文件
            }
            plotRed.Enabled = false;//設定「描紅」按鈕為不可用狀態
            keyWord.Focus();//使焦點位於文字框中
        }
        private void FetchFile(string path)
        {
            ...
            richTextBox1.Text = UTF_8Reader.ReadToEnd();//在RichTextBox控制元件中顯示從字節流中讀取的字符
        }
        ...
        private void unfold_Click(object sender, EventArgs e)
        {
            ...
            UTF_8File.Filter = "TXT文件（*.TXT）|*.TXT";//設定打開對話框的打開過濾參數
            if (UTF_8File.ShowDialog() == DialogResult.OK)//當在打開對話框中點擊「打開」按鈕時
            {
                richTextBox1.LoadFile(UTF_8File.FileName, RichTextBoxStreamType.PlainText);//從指定的位置載入TxT文件
                MessageBox.Show("讀取成功！", "提示訊息", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);//彈出讀取成共的訊息提示
        ...
        private void plotRed_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("沒有要搜尋的結果", "提示訊息", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);//彈出對應的訊息提示
                keyWord.Clear();//清空文字框中的內容
                flag = 0;//重新為flag賦值
            }
            else//當在文件中存在對應的關鍵字時
            {
                richTextBox1.Select(flag, keyWord.Text.Length);//在RichTextBox控制元件中搜索關鍵字
        ...
        {
            if (keyWord.Text == "" || keyWord.Text == null)//當文字框中不存在內容或者內容為空時
            {
                plotRed.Enabled = false;//設定「描紅」按鈕為不可用狀態
            }
            else//當文字框中存在內容時
