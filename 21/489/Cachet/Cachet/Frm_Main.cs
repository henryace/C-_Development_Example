...
namespace Cachet
{
    public partial class Frm_Main : Form
    {
        ...
        Font Var_Font = new Font("Arial", 12, FontStyle.Bold);//定義字串的字體樣式
        Rectangle rect = new Rectangle(10, 10, 160, 160);//實例化Rectangle類

        private void button1_Click(object sender, EventArgs e)
        {
            int tem_Line = 0;//記錄圓的直徑
            int circularity_W = 4;//設定圓畫筆的粗細
            if (panel1.Width >= panel1.Height)//如果panel1控制元件的寬度大於等於高度
                tem_Line = panel1.Height;//設定高度為圓的直徑
            else
                tem_Line = panel1.Width;//設定寬度為圓的直徑
            rect = new Rectangle(circularity_W, circularity_W, tem_Line - circularity_W * 2, tem_Line - circularity_W * 2);//設定圓的繪製區域
            Font star_Font = new Font("Arial", 30, FontStyle.Regular);//設定星號的字體樣式
            string star_Str = "★";
            Graphics g = this.panel1.CreateGraphics();//實例化Graphics類
            g.SmoothingMode = SmoothingMode.AntiAlias;//消除繪製圖形的鋸齒
            g.Clear(Color.White);//以白色清空panel1控制元件的背景
            Pen myPen = new Pen(Color.Red, circularity_W);//設定畫筆的顏色
            g.DrawEllipse(myPen, rect); //繪製圓 
            SizeF Var_Size = new SizeF(rect.Width, rect.Width);//實例化SizeF類
            Var_Size = g.MeasureString(star_Str, star_Font);//對指定字串進行測量
            //要指定的位置繪製星號
            ...
            Var_Size = g.MeasureString("專用章", Var_Font);//對指定字串進行測量
            //繪製文字
            ...
            int len = tempStr.Length;//取得字串的長度
            float angle = 180 + (180 - len * 20) / 2;//設定文字的旋轉角度
